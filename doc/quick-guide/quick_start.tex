...
the accelerated code makes use of CPU multithreading. Hence, setting the environment properly is essential, and \command{pc\_start} and \command{pc\_run} do this for you when invoked in a SLURM job with a proper setting of the \code{srun} parameter \code{cpus-per-task} (see below). Without SLURM, you have to set
...
When using \command{start.csh} and \command{run.csh}, it is recommended to additionally export the settings
\begin{verbatim}
...
\end{verbatim}
while, again, \code{pc\_start/pc\_run} will do this for you.
...
The best speedups are obtained for subdomain (= per-process) grid sizes of the order of $256^3$ or bigger. 
